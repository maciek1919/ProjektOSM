...
public class zabezpieczenia {
	...
	public static int sprawdzBadanie(String lleukocytow, String lneutrofili, String lerytrocytow, JTable tabela, String sData, Date data) {
		...
		if(!lleukocytow.matches("[0-9]+") || !lneutrofili.matches("[0-9]+") || !lerytrocytow.matches("[0-9]+")) {
			JOptionPane.showMessageDialog(null, "Zly format wynik�w bada�", "Warning", JOptionPane.WARNING_MESSAGE);
			return 2;
		}
	...
	public static int sprawdzPacjenta(String imie, String nazwisko, String pesel, JTable Tabela, boolean isMale) {
		  ...
		  /* if(!isMale && liczbaM(pesel)) {
		   ...
		   int i = 0;
		   for (Pacjent pacjentVectorList1 : Presenter.pacjentVectorList) { //p�tla foreach
	   ...
	   public static void errorMessage(int check) {
		   ...
		   if(check == 1) {
	        ...
	        if(check == 3) {
	     	   ...
	     	   JOptionPane.showMessageDialog(null, "Zły format numeru Pesel", "Warning", JOptionPane.WARNING_MESSAGE);
	        }
	       ...
	       if(check == 6) {
	    	   JOptionPane.showMessageDialog(null, "Błędna 10 cyfra numeru Pesel", "Warning", JOptionPane.WARNING_MESSAGE);
	       }
	       if(check == 7) {
